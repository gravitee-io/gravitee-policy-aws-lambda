...
^.^|integer
^.^|50000 (AWS SDK default)

.^|requestTimeoutMs
^.^|
|The amount of time to wait (in milliseconds) for a single HTTP request to complete before giving up. If not set, the default value of the AWS SDK is used.
^.^|integer
^.^|0 - disabled (AWS SDK default)

.^|clientExecutionTimeoutMs
^.^|
|The total amount of time (in milliseconds) for the client to complete the execution of an API call, including retries and backoff. If not set, the default value of the AWS SDK is used.
^.^|integer
^.^|0 - disabled (AWS SDK default)
